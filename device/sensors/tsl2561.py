...
timing = (
    2  # current integration time: 0=13.7ms, 1=101ms, 2=402ms [dynamically selected]
)
...
class TSL2561:
    def readRegister(self, address):
        try:
            ...
            if debug:
                print(
                    "TSL2561.readRegister: returned 0x%02X from reg 0x%02X"
                    % (byteval, address)
                )
            return byteval
        except IOError:
    ...
    def writeRegister(self, address, val):
        try:
            ...
            if debug:
                print(
                    "TSL2561.writeRegister: wrote 0x%02X to reg 0x%02X" % (val, address)
                )
        except IOError:
    ...
    def setTintAndGain(self):
        ...
        if timing == 0:
            ...
            timing_ms = 402
        self.writeRegister(TSL2561_Timing, timing | gain << 4)
    ...
    def readLux(self):
        ...
        if debug:
            print(
                "TSL2561.readVisibleLux: channel 0 = %i, channel 1 = %i [gain=%ix, timing=%ims]"
                % (channel0, channel1, gain_m, timing_ms)
            )
    ...
    def readVisibleLux(self):
        ...
        if (
            (timing == 0 and (channel0 > 5000 or channel1 > 5000))
            or (timing == 1 and (channel0 > 37000 or channel1 > 37000))
            or (timing == 2 and (channel0 > 65000 or channel1 > 65000))
        ):
            # overflow
            return -1

        return self.calculateLux(channel0, channel1)
...
def main():
    ...
    while True:
        print(
            "Lux: %i [Vis+IR=%i, IR=%i @ Gain=%ix, Timing=%.1fms]"
            % (sensor.read(), channel0, channel1, gain_m, timing_ms)
        )
        sleep(1)
